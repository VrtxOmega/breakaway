// Breakaway App - Core
const APP = { user: null, credentials: [], view: 'landing', aiMode: null, pasteCount: 0, xp: 0, streak: 0, lastCompleted: null, stroopRound: 0, stroopCorrect: 0, stroopTotal: 0, speedparsePhase: 'flash', speedparseAnswers: [] };
...
function startChallenge(id){
  ...
  APP.activeChallenge=c;APP.pasteCount=0;
...
async function submitQuiz(){
  ...
  const hash=await sha256(JSON.stringify({id:c.id,answers:APP.quizAnswers,score,time:elapsed,aiMode:APP.aiMode,pasteCount:APP.pasteCount,ts:Date.now()}));
  const cred={id:hash.slice(0,16),challengeId:c.id,domain:c.domain,title:c.title,score,time:elapsed,hash,aiMode:APP.aiMode,pasteCount:APP.pasteCount,timestamp:new Date().toISOString()};
...
async function submitCode(){
  ...
  const elapsed=Math.floor((Date.now()-APP.startTime)/1000);
  const hash=await sha256(JSON.stringify({id:c.id,code,score,time:elapsed,aiMode:APP.aiMode,pasteCount:APP.pasteCount,ts:Date.now()}));
  const cred={id:hash.slice(0,16),challengeId:c.id,domain:c.domain,title:c.title,score,time:elapsed,hash,aiMode:APP.aiMode,pasteCount:APP.pasteCount,timestamp:new Date().toISOString()};
...
async function submitValidate(){
  ...
  const elapsed=Math.floor((Date.now()-APP.startTime)/1000);
  const hash=await sha256(JSON.stringify({id:c.id,answers,score,time:elapsed,aiMode:APP.aiMode,pasteCount:APP.pasteCount,ts:Date.now()}));
  const cred={id:hash.slice(0,16),challengeId:c.id,domain:c.domain,title:c.title,score,time:elapsed,hash,aiMode:APP.aiMode,pasteCount:APP.pasteCount,timestamp:new Date().toISOString()};
...
async function submitPrompt(){
  ...
  const elapsed=Math.floor((Date.now()-APP.startTime)/1000);
  const hash=await sha256(JSON.stringify({id:c.id,prompt,score,time:elapsed,aiMode:APP.aiMode,ts:Date.now()}));
  const cred={id:hash.slice(0,16),challengeId:c.id,domain:c.domain,title:c.title,score,time:elapsed,hash,aiMode:'native',pasteCount:0,timestamp:new Date().toISOString()};
...
async function finishStroop(){
  ...
  const elapsed=Math.floor((Date.now()-APP.startTime)/1000);
  const hash=await sha256(JSON.stringify({id:c.id,correct:APP.stroopCorrect,total:APP.stroopTotal,avgTime,score,aiMode:APP.aiMode,ts:Date.now()}));
  const cred={id:hash.slice(0,16),challengeId:c.id,domain:c.domain,title:c.title,score,time:elapsed,hash,aiMode:APP.aiMode,pasteCount:0,timestamp:new Date().toISOString(),
...
async function submitSpeedParse(){
  ...
  const elapsed=Math.floor((Date.now()-APP.startTime)/1000);
  const hash=await sha256(JSON.stringify({id:c.id,answers:APP.speedparseAnswers,score,time:elapsed,aiMode:APP.aiMode,ts:Date.now()}));
  const cred={id:hash.slice(0,16),challengeId:c.id,domain:c.domain,title:c.title,score,time:elapsed,hash,aiMode:APP.aiMode,pasteCount:0,timestamp:new Date().toISOString(),
...
async function submitAIValidate(){
  ...
  const elapsed=Math.floor((Date.now()-APP.startTime)/1000);
  const hash=await sha256(JSON.stringify({id:c.id,answers,score,found,time:elapsed,aiMode:APP.aiMode,pasteCount:APP.pasteCount,ts:Date.now()}));
  const cred={id:hash.slice(0,16),challengeId:c.id,domain:c.domain,title:c.title,score,time:elapsed,hash,aiMode:APP.aiMode,pasteCount:APP.pasteCount,timestamp:new Date().toISOString(),
...
function showResults(cred,detail){
  ...
  const integrityHtml=cred.aiMode==='solo'&&cred.pasteCount>0?'<div class="integrity-signal"><span class="integrity-icon">📋</span> '+cred.pasteCount+' paste event'+(cred.pasteCount>1?'s':'')+' detected</div>':'';
